...
SEND_INFO = CommandInfo(
    ...
    details = f"Send specified amount to specified address." +
                f"\nExample `{config.Config.instance().command_prefix}send 10 {Env.currency_symbol().lower()[:5]}_3o7uzba8b9e1wqu5ziwpruteyrs3scyqr761x7ke6w1xctohxfh5du75qgaj` - Sends 10 {Env.currency_symbol()}"
)
SENDMAX_INFO = CommandInfo(
    ...
    details = f"Send entire balance to specified address." +
                f"\nExample `{config.Config.instance().command_prefix}sendmax {Env.currency_symbol().lower()[:5]}_3o7uzba8b9e1wqu5ziwpruteyrs3scyqr761x7ke6w1xctohxfh5du75qgaj` - Sends entire {Env.currency_symbol()} balance"
)
...
class AccountCog(commands.Cog):
    ...
    async def register_cmd(self, ctx: Context):
        ...
        embed.set_author(name=user_address, icon_url="https://cdn.discordapp.com/emojis/843275907639541770.png" if Env.banano() else "https://github.com/bbedward/graham_discord_bot/raw/master/assets/nano_logo.png")
    ...
    def format_balance_message(self, balance_raw: int, pending_raw: int, pending_send_db: int, pending_receive_db: int) -> discord.Embed:
        embed = discord.Embed(colour=0xFFFFFF if Env.banano() else discord.Colour.dark_blue())
        embed.set_author(name="Balance", icon_url="https://cdn.discordapp.com/emojis/843275907639541770.png" if Env.banano() else "https://github.com/bbedward/graham_discord_bot/raw/master/assets/nano_logo.png")
